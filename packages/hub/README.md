...
**Core Concepts:**
- **Farm** - Represents a legal business entity or physical farm registered in the cooperative
...
The Hub provides a secure onboarding workflow for adding new farms:

1. **Admin Creates Farm**: An administrator calls `CoopIdentity.add_new_farm/1` with farm and farmer details
...
3. **Credential Distribution**: Admin receives plaintext credentials (shown only once) to securely communicate to the farm
...
For detailed information, see [Farm Onboarding Documentation](docs/farm-member-onboarding.md).
